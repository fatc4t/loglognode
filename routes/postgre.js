...
router.post("/GetLikesList", async function (req, res) {
  ...
  try {
    const query1 = {
      text: `SELECT c.*, mst0010.shop_nm FROM coupons AS c JOIN (SELECT unique_coupon_cd FROM coupons_liked WHERE user_cd = $1) AS cl ON c.unique_coupon_cd = cl.unique_coupon_cd
